...
import 'emp_auth_remote_data_source.dart';

class EmpAuthRemoteDataSourceImpl implements EmpAuthRemoteDataSource {
  final Dio dio;
  final AuthLocalDataSource local;

  EmpAuthRemoteDataSourceImpl({required this.dio, required this.local});
  ...
  Future<EmpUserModel> register(
    ...
    try {
      final response = await dio.post(
        ...
        data: {'email': email, 'name': name, 'password': password, 'HaveVacancies': 'Have vacancies'},
      );
      ...
      final user = EmpUserModel(id: "id", email: "email", name: "name");
  ...
  @override
  Future<EmpUserModel> login(String email, String password) async {
    try {
      final response = await dio.post(
        ApiConstants.login,
      ...
      return user;
      // Return EmpUserModel
      // return EmpUserModel.fromJson(userJson);
    } on DioException catch (e) {
      throw Exception(_handleError(e));
    }
  }
  ...
  @override
  Future<EmpUserModel> changePassword(String password) async {
    try {
      final response = await dio.post(
